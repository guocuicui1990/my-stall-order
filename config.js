        ...
        Object.keys(localShops).forEach(shopId => {
            if (!window.shopConfigs[shopId]) {
                window.shopConfigs[shopId] = {
                    ...
                    dishes: localShops[shopId].dishes || [] // 使用本地存储的菜品数据
                };
            }
        });
...
async function loadShopDishesFromSupabase(shopId) {
    try {
        ...
        if (!window.supabaseClient) {
            ...
            // 如果本地也没有，返回默认菜品
            return window.shopConfigs['default_shop'].dishes || [];
        }
    ...
    if (!window.shopConfigs[shopId]) {
        ...
        if (localShops[shopId]) {
            window.shopConfigs[shopId] = {
                ...
                logo: 'images/logo.png',
                qrcode: 'images/qrcode.jpg',
                dishes: localShops[shopId].dishes || [] // 从本地存储加载菜品
            };
        }
    }
...
window.shopConfigs = {
    ...
    // 其他商家配置将由loadRegisteredShops函数动态添加
    // 或者在此处手动添加已注册的商家配置
};

// 获取当前商家ID - 管理员专用版本（完全从URL获取）
function getCurrentShopId() {
    // 只从URL参数获取，如 ?shop=shop_id_123
    ...
    if (!shopId || !window.shopConfigs[shopId]) {
        // 尝试从本地存储查找
        const localShops = JSON.parse(localStorage.getItem('registered_shops') || '{}');
        if (Object.keys(localShops).length > 0 && !shopId) {
            // 如果只有一个商家，默认使用该商家
            const shopIds = Object.keys(localShops);
            if (shopIds.length === 1) {
                shopId = shopIds[0];
            }
        }
        ...
        for (const shopId in localShops) {
            if (!window.shopConfigs[shopId]) {
                ...
                if (fullConfig && fullConfig[shopId]) {
                    // 使用完整配置
                    window.shopConfigs[shopId] = fullConfig[shopId];
                } else {
                    // 使用基本信息创建配置
                    window.shopConfigs[shopId] = {
                        name: shopData.name,
